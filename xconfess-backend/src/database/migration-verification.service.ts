  ...
  private async verifyConfessionsSchema() {
    try {
      ...
      if (!columnNames.includes('search_vector')) {
        this.logger.error(
          'CRITICAL: Column "search_vector" missing on "anonymous_confessions" table!',
        );
      } else {
      ...
      if (!columnNames.includes('view_count')) {
        this.logger.error(
          'CRITICAL: Column "view_count" missing on "anonymous_confessions" table!',
        );
      } else {
      ...
      if (!indexNames.includes('idx_confession_search_vector')) {
        this.logger.error(
          'CRITICAL: Index "idx_confession_search_vector" missing on "anonymous_confessions" table!',
        );
      } else {
        this.logger.log(
          'Verified: Index "idx_confession_search_vector" exists.',
        );
      }
    } catch (error) {
      this.logger.error(
