...
export class ContractService {
  ...
  async invokeContract(
    ...
    try {
      ...
      const encodedArgs = this.encodeContractArgs(invocation.args); // returns args as-is
  ...
  async verifyConfession(confessionHash: string): Promise<number | null> {
    try {
      ...
      // Decode timestamp
      const timestamp = StellarSDK.scValToNative(result);
  ...
  /**
   * FIXED: Proper ScVal encoding for contract parameters
   */
  private encodeContractArgs(args: any[]): StellarSDK.xdr.ScVal[] {
    return args.map((arg) => {
  ...
  // TODO: Optimize contract parameter encoding for complex types in future
}import { Injectable, Logger } from '@nestjs/common';
import * as StellarSDK from '@stellar/stellar-sdk';
import { StellarConfigService } from './stellar-config.service';
import { TransactionBuilderService } from './transaction-builder.service';
...
export class ContractService {
  ...
  private encodeContractArgs(args: any[]): any[] {
    // WRONG: Just returning args as-is
    return args;
  }
